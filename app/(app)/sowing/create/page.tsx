...
export default function CreateSowingPage() {
  ...
  async function onSubmit(data: CreateSowingFormData) {
    setSubmitting(true);
    setServerError(null);
    ...
    try {
      ...
      if (data.greenhouse === "GREENHOUSE") {
        sowingDto.numberOfTrays = data.numberOfTrays;
        sowingDto.seedsPerTray = data.seedsPerTray ?? DEFAULT_SEEDS_PER_TRAY;
        const numTrays = Number(data.numberOfTrays ?? 0);
        const seedsPerTray = Number(
          data.seedsPerTray ?? DEFAULT_SEEDS_PER_TRAY,
        );
        sowingDto.quantityUsed = numTrays * seedsPerTray;
      } else {
        sowingDto.quantityUsed = Number(data.quantityUsed ?? 0);
      }

      if (data.remarks) {
        sowingDto.remarks = data.remarks;
      }

      await createSowing(sowingDto);
      router.push("/sowing");
    } catch (err) {
      setServerError(
        err instanceof Error ? err.message : "Failed to create sowing record",
      );
    } finally {
      setSubmitting(false);
    }
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 
                  ...
                  focus:outline-none focus:ring-2 focus:ring-green-500 text-sm"
              >
                <option value="GREENHOUSE">Greenhouse (Plastic Tunnel)</option>
                <option value="FIELD">Field (Direct Sowing)</option>
              </select>
              {errors.greenhouse && (
                <p className="mt-1 text-xs text-red-600">
